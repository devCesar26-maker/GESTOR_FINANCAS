...
def test_service_pagar_fatura_sucesso(fatura_pendente):
    res = services.pagar_fatura(fatura_pendente)
...
@pytest.mark.django_db
def test_endpoint_pagar_fatura_conflito_409(auth_client, fatura_pendente):
    auth_client.post(f"{FATURAS_URL}{fatura_pendente.id}/pagar/")
    # Pagamento repetido -> 409 Conflict
    response = auth_client.post(f"{FATURAS_URL}{fatura_pendente.id}/pagar/")
